...
정상 응답은 공통 응답 객체 `ApiResponse<T>`를 `ResponseEntity`로 감싸 반환한다.
...
- 인증 사용자 정보 추출
- Service 호출
- SuccessCode를 사용한 `ApiResponse<T>` 생성 및 `ResponseEntity` 반환
...
## 공통 응답
- 모든 정상 응답은 `ResponseEntity<ApiResponse<T>>`를 반환한다.
- HTTP 상태 코드는 도메인의 SuccessCode에서 가져온다.
- 도메인의 `XXXSuccessCode`를 사용한다.
- 메시지와 응답 코드를 문자열로 직접 작성하지 않는다.

```java
return ResponseEntity.status(MemberSuccessCode.MEMBER_INFO_FETCH_SUCCESS.getStatus())
...
```java
return ResponseEntity.status(MemberSuccessCode.MEMBER_WITHDRAWAL_SUCCESS.getStatus())
    .body(ApiResponse.onSuccess(
...
public interface MemberControllerDocs {
    ...
    ResponseEntity<ApiResponse<MemberResDTO.Detail>> getMember(Long memberId);
}
```
...
public class MemberController implements MemberControllerDocs {
    ...
    @Override
    @GetMapping("/{memberId}")
    public ResponseEntity<ApiResponse<MemberResDTO.Detail>> getMember(
...
## 금지 사항
- `ResponseEntity` 없이 HTTP 상태 코드가 필요한 성공 응답 처리
- Repository 직접 호출
- Entity 생성 또는 수정
